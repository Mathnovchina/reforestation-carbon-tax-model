...
calibrateModel<-function(sysMinDist,dataParameters,dataVariables,dataSeries,nbOfRuns=1,plotOptim=T,resultFile=NULL,printEvol=T,timeframe){
	...
	# write.csv(cbind(sys$parms,fixedParms,lowerParms,upperParms),file="parm.csv")
	...
	if(plotOptim){
		...
		par(mfrow=c(2,length(targetNames)))
		for(i in 1:length(targetNames)){
			name=targetNames[i]
			matplot(timeframe,cbind(target[,targetNames[i]],simul[targetNames[i]]),type='l',main=targetNames[i],ylab="",xlab="")
			legend("topleft",lty=1:2,col=1:2,legend=c("Observations","Simulations"),bty='n')
		}
		for(i in 1:length(targetNames)){
			name=targetNames[i]
			matplot(timeframe,100*cbind(simul[targetNames[i]]-target[,targetNames[i]])/target[,targetNames[i]],type='l',main=targetNames[i],ylab="% erreur",xlab="")
		}
	}
	return(allPars)
}